...
export default ({ item, actions, state }) => {
	...
	return <div title={state.isEditor === true && "Drag me to change my position"} key={item.id} active={item.isActive} className={`
			swappable
			${item.isDragging === true ? "is-dragged" : ""}
			${item.isSlipZone === true ? "is-slipzone" : ""}`} className={cc({
				transition: true,
				...
				"bg-green": state.isEditor === true && state.isSliderFullview === false && item.id === state.currentItem.id,
			...
			})} onclick={(e) => actions.setCurrentSlide({ id: item.id, position: item.position })} ondragstart={(e) => {
			...
			}} ondragover={(e) => e.preventDefault()} ondragend={(e) => {
